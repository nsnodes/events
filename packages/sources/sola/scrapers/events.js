...
/**
 * Scrape discover page to get all group/community URLs
 * @param {Object} options - Configuration options
 * @param {boolean} options.headless - Run browser in headless mode (default: true)
 * @returns {Promise<Object>} Result with group URLs
 */
export async function scrapeDiscoverPage(options = {}) {
  ...
  try {
    ...
    const groupUrls = await page.evaluate(() => {
      // Get all event group links - /event/{slug} (not /event/detail/{id})
      const links = Array.from(document.querySelectorAll('a[href*="/event/"]'));
      const urls = new Set();

      links.forEach(a => {
        const href = a.href;
        // Include group pages, not detail pages
